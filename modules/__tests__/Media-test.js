...
const renderAndHydrate = markup => {
  const serverMarkup = serverRenderStrict(markup);
  // Creating our own container so that we can put the static server-rendered markup
  // into it and then hydrate against that.
  const serverContainer = document.body.appendChild(document.createElement('div'));
  serverContainer.innerHTML = serverMarkup;
  return renderStrict(
    markup,
    { hydrate: true, container: serverContainer },
  );
}

describe.only('Hydration tests', () => {
  let originalMatchMedia;
  ...
  describe.only("when no defaultMatches is defined", () => {
    it('should throw no hydration warning if there is no client match', () => {
      window.matchMedia = createMockMediaMatcher(false);
      renderAndHydrate((
        <Media query={queries.sm}>
          <div>matched</div>
    ...
    it('should throw no hydration warning if there is a subsequent client match', () => {
      window.matchMedia = createMockMediaMatcher(true);
      renderAndHydrate((
        <Media query={queries.sm}>
          <div>matched</div>
